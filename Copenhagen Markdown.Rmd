...
cph <- read.csv(here("data/copenhagen_univariable.csv"), stringsAsFactors = FALSE) 
```
...
cph <- read.csv(here("data/copenhagen_stratified.csv"), stringsAsFactors = FALSE) 
```
...
outputs <- vector(mode = "list", length = length(vars))
...
# Create an empty list to save the output of the loop
outputs2 <- vector(mode = "list", length = length(vars))
...
for (var in vars) {
  ...
  resultstable <- rbind(mh$massoc$RR.crude.wald,
                        mh$massoc$RR.strata.wald,
                        mh$massoc$RR.mh.wald)
  rownames(resultstable) <-
    c("Crude", "Strata 1", "Strata 0", "Adjusted")

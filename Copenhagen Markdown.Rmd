...
### Reading in datasets
Open the data set **copenhagen_raw.dta** using haven's `read_dta()` command.  
We will then convert the labels to factors and save it as a comma-separated values (csv) file. 
...
There is two possible ways to do this. The first is to use the wilcox.test again, and the second is to calculate ORs using the `epiR::epi.2by2()` function. Using the `epiR::epi.2by2()` function for dose responses is a bit more dense (traditionally this would be done using regression), so we have not shown it here; if you would like to see the code for this it is available in the appendix.
...
cph <- read.csv(here::here("data", "copenhagen_stratified.csv"), stringsAsFactors = FALSE) 
```
...
You can then extract various outputs from the `epi.2by2()` table for piecing together your own output. 
...
strata_risk <- function(var, case, strat) {
  ...
  # Use the epi.2by2 function to calculate RRs (by stating method = "cohort.count")
  mh <- epiR::epi.2by2(a, method = "cohort.count")
...
This is easily done, if you have saved each of your analyses in a seperate script (e.g. one for cleaning, descriptive, univariable and stratified each), you can create a master script by using the `rmarkdown::render()` function. 

Open a new R file and save it as `runAll.R`. Inside of this script, put the following:

```
rmarkdown::render(here::here("reports", "01-intro-clean.R"))
rmarkdown::render(here::here("reports", "02-descriptive-stats.R"))
rmarkdown::render(here::here("reports", "03-univariate-analyses.R"))
rmarkdown::render(here::here("reports", "04-stratified-analyses.R"))
```
...
cph <- read_dta(here("data", "copenhagen_stratified.dta"))
...
```{r, eval = FALSE}
library("readxl")
cph <- read_excel(here("data", "copenhagen_stratified.xlsx"))
```
...
rownames(a) <-
  c("student", "teacher", "1", "2", "3", "male", "female", "total")

colnames(a) <-
  c("Number", "% of Total", "Number of cases", "Attack rate (in %)")
...
dir.create("results")
write.csv(z, file = here("results", "case-results.csv")

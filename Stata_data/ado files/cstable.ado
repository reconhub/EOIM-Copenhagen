...
quietly foreach var of varlist `varlist'  {
	...
	local iRow = `iRow'+1
	if `iRow'==1{
		local finalvarlist = "`var'" // will be removed at the end
		continue
		}
	...
	local ref = "`var'"
	local subvarlist = "`var'"
	// test if variable is bynary
	tab `var', matrow(_tempvar)
	local _nrow = r(r)
	// drop if too many
	if `_nrow' > 10 {
		noisily : display as result "`var' dropped because of too many categories or continuous"
		local iRow = `iRow'-1
		continue
		}
	if `_nrow' > 2 {
		capture drop _I`var'*
		local subvarlist = ""
		local _indice = el(_tempvar,1,1)
		noisily : display as result "`var' is categorical, reference assumed `var'==`_indice'"
		forvalues jrow = 2(1)`_nrow'{
			local _indice = el(_tempvar,`jrow',1)
			gen _I`var'_`_indice' = 0
			label variable _I`var'_`_indice' "`var' == `_indice'"
			replace _I`var'_`_indice' = 1 if `var' == `_indice'
			local subvarlist = "`subvarlist'"+"_I`var'_`_indice' "
			}
		}

	local iRow = `iRow'-1
	
	foreach subvar of varlist `subvarlist'  {
	local iRow = `iRow'+1
	local finalvarlist = "`finalvarlist' `subvar'"
	// Get Exposed, Cases, AR	
	count if `subvar' == 1 & `1' != .
	local _Totexp = r(N)
	count if `subvar' == 1 & `1' == 1
	local _Casexp = r(N)
	local _ARexp = `_Casexp' / `_Totexp' * 100
	local _Ncasexp = `_Totexp' - `_Casexp'
	...
	local _Ncasunexp = `_Totunexp' - `_Casunexp'
	...
	// cs `1' `var' `exact'
	csi `_Casexp' `_Casunexp' `_Ncasexp' `_Ncasunexp' `exact'
...
forvalues jrow = 2(1)`iRow'{
   ...
   local _rowname : word `_iorder' of `finalvarlist'
